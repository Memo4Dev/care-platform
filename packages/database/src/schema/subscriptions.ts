import { boolean, check, index, numeric, pgSchema, text, timestamp, unique, uniqueIndex, uuid } from 'drizzle-orm/pg-core';
...
export const subscriptionStatusEnum = subscriptionSchema.enum('subscription_status', SUBSCRIPTION_STATUSES);
...
export const subscriptions = subscriptionSchema.table('subscriptions', {
  ...
  check('subscriptions_period_valid', sql`${table.currentPeriodEnd} > ${table.currentPeriodStart}`),
  index('subscriptions_organization_status_period_end_idx').on(table.organizationId, table.status, table.currentPeriodEnd),
  uniqueIndex('subscriptions_one_commercial_per_organization_unique').on(table.organizationId).where(sql`${table.status} in ('TRIAL', 'ACTIVE', 'PAST_DUE', 'SUSPENDED')`),
  index('subscriptions_plan_id_idx').on(table.planId),
]);

/** Immutable period / plan history. A correction is a new row, never an update. */
export const subscriptionPeriods = subscriptionSchema.table('subscription_periods', {
  id: idColumn(),
  ...
  unique('subscription_periods_subscription_effective_unique').on(table.subscriptionId, table.effectiveAt),
  index('subscription_periods_subscription_effective_idx').on(table.subscriptionId, table.effectiveAt),
]);
