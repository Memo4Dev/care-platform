...
Phase: M5 COMPLETE — MERGED into main (merge commit e7149d8); M5 RELEASE GATE: PASS. Awaiting explicit M6 kickoff approval; M6 implementation NOT started.
Milestone: M5 (Sales & POS Core) — COMPLETE, merged. Next milestone: M6 (SCHEDULED, awaiting kickoff; NOT started).
Active task: ORCH-001 — Persistent Milestone Todo & Session Recovery hardening (completed; committed as part of the orchestration/state fix). Post-M5 state reconciled: M5 tasks preserved DONE in `docs/state/TASKS.md` under "Milestone COMPLETE — M5"; M5-SEC-001/002 recorded and scheduled into M6 (not in progress); the next milestone is initialized as a scheduled, awaiting-kickoff set (M6-001 + M5-SEC-001/002) without destroying M5 history. M6 implementation is explicitly out of scope until the human approves the kickoff.
CI: Post-merge CI on `main` at `e7149d8` — run `33756419219` **GREEN (success)** (job `quality`). M5 pre-merge CI on `feat/m5-sales-pos-core` also green: run `33753480107` (head `7c7324a`) and run `33754811394` (head `4dacb32`), both including the 2 Redis/BullMQ integration tests with authenticated Redis (not skipped). Historical staging credential rotation remains required — `METRICS_BEARER_TOKEN` must be rotated at next staging deploy.
Branch: main (merged from feat/m5-sales-pos-core)
...
**M5 merged into main.** Merged via `--no-ff` (`merge: feat/m5-sales-pos-core into main`) as commit `e7149d8`; post-merge CI on `main` (`e7149d8`) GREEN run `33756419219`. M5 is COMPLETE and its tasks (M5-003…M5-010) preserved DONE in `docs/state/TASKS.md` under "Milestone COMPLETE — M5". Post-M5 reconciliation (ORCH-001) initialized the next milestone (M6) as a scheduled, awaiting-kickoff set (M6-001 + M5-SEC-001/002) without destroying M5 history; M6 implementation is explicitly out of scope pending human approval.

## M5 milestone summary

M5 delivered the Sales & POS Core: POS Cart aggregate (create, items, `/save`), hold/resume reservation lifecycle with configurable TTL (ADR-0010), barcode/availability/Pricing-quote integration into Cart, checkout to an immutable `PENDING_PAYMENT` Sale snapshot (ADR-0012), and an exactly-once internal trusted completion/cancellation boundary behind a signed `SALES_INTERNAL_BEARER_TOKEN` with token-carried org scope — never a caller-supplied header. Concurrency coverage (4 tests) proves exactly-once under real PostgreSQL races: concurrent double-complete (reservation CONSUMED + FIFO/stock consumed exactly once), concurrent double-cancel (reservation RELEASED exactly once), and cancel-vs-complete race (exactly one terminal state; loser 409 `SALE_INVALID_STATE`; reservation + stock reflect the single winner). Full Swagger response schemas and an OpenAPI contract test assert the four Sales routes with `tenant-bearer` vs `internal-bearer` security and required `Idempotency-Key`/`If-Match`/`saleId`. Independent security review: CONDITIONAL PASS (no blockers); correctness/architecture review: PASS (all M5 exit criteria met). Final gates green: typecheck PASS, lint+prettier PASS, `pnpm test` **614 passed**, native-PG integration **466 passed / 2 Redis CI-required** incl. Sales HTTP (7) + concurrency (4).
